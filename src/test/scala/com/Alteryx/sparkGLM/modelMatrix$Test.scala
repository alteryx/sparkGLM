...
class modelMatrix$Test extends FunSuite {
  val conf = new SparkConf().setMaster("local").setAppName("modelMatrixTest")
  val sc = new SparkContext(conf)
  val sqlCtx = new SQLContext(sc)

  test("modelMatrixWithMixedTypes") {
    val testDF = modelMatrix(dummyDF(sqlCtx))
  ...
  test("modelMatrixWithNumOnly") {
    val testDF = modelMatrix(dummyDF(sqlCtx).select("numField", "intField"))
  ...
  test("modelMatrixWithStrOnly") {
    val testDF = modelMatrix(dummyDF(sqlCtx).select("strField"))
    assert(testDF.columns.length == 2)
  ...
  test("modelMatrixLinearRegData") {
    val rawDF = mixedDF(sqlCtx).select("intercept", "x1", "x2", "x3", "x4", "x5", "x6", "x7", "y")
